...
education = section "教育经历" "Education"
  ...
  , cn $ "专业：计算机科学（本科），预计毕业日期： 2023 年 1 月， " ++ gpa
...
aya = paragraph
  ...
  , itemize
    [ cn "\\item 支持依值类型、依值模式匹配和隐式参数的推导。"
    , en "\\item Supports dependent types, dependent pattern matching, and implicit arguments."

    , cn "\\item 支持可视化类型检查的完整证明树，且能在证明树中悬浮鼠标高亮对应位置的源码。"
    , en "\\item Supports visualization of the type checking traces with source code highlight on hovering."
    ]
  ]
