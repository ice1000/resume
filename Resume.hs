...
singularityData = paragraph
  [ datedSection2 (date "2022" "7" ~~ date "2023" "7") $ paragraph
  ...
  , itemize
    [ elab "\\item Proposed an overhaul of the query plan AST design, which better facilitates the enum feature implemented in the Rust programming language."
    , cn "\\item 提案重构查询计划语法树的设计以更好地使用 Rust 编程语言的枚举功能。"

    , en "\\item Implemented a pretty printing framework for trees with line fitting and Unicode art. Integrated into SQL explain."
    , cn "\\item 实现将树状数据使用 Unicode 艺术可视化的算法，会计算行宽并支持智能断行，集成到 SQL explain 中。"
    ]
  ]
...
la461 :: Resume
la461 = paragraph
  [ datedSection2 (date "2022" "8" ~~ date "2022" "12") $ paragraph
...
resume = paragraph
  ...
  , section "其他" "Miscellaneous"
    ...
    , en $ seRep ++ " reputation, also active on " ++ stackex "other StackExchange sites"
